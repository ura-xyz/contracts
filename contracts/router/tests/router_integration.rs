...
use ura::contracts::router::{ExecuteMsg, InstantiateMsg, SwapOperation};
use ura::structs::asset_info::token_asset_info;

fn router_contract() -> Box<dyn Contract<Empty>> {
    Box::new(ContractWrapper::new_with_empty(
        ura_router::contract::execute,
        ura_router::contract::instantiate,
        ura_router::contract::query,
    ))
}
...
fn router_does_not_enforce_spread_assertion() {
    ...
    let router = app
        .instantiate_contract(
            ...
            owner.clone(),
            &InstantiateMsg {
                ura_factory: helper.factory.to_string(),
            },
            &[],
            "router",
            None,
...
fn test_swap_route() {
    ...
    let owner = Addr::unchecked("owner");
    let mut helper = FactoryHelper::init(&mut app, &owner);
    let ura = instantiate_token(&mut app, helper.cw20_token_code_id, &owner, "ura", None);
    ...
    for (a, b, typ, liq) in [
        (&ura, &inj, PairType::Xyk, 100_000_000000),
    ...
    // route: ura -> inj, atom -> osmo
    let swap_operations = vec![
        SwapOperation {
            offer_asset_info: AssetInfo::Token {
                contract_addr: ura.clone(),
            },
    ...
    // swap ura for osmo
    // route: ura -> inj, inj -> osmo, osmo -> atom, atom -> osmo
    let swap_operations = vec![
        SwapOperation {
            offer_asset_info: AssetInfo::Token {
                contract_addr: ura.clone(),
    ...
    mint(&mut app, &owner, &ura, swap_amount.u128(), &user).unwrap();
    ...
    let balance_res: BalanceResponse = app
        ...
        .unwrap();
    println!("ura router balance: {:?}", balance_res.balance);
